        ...
        if (state && action === 'REPLACE') {
            ...
            return (
                <div className={`alert ${state.status === 'success' ? 'alert-success' : 'alert-danger'}`} role="alert">
                    {`[${state.time}] ---`} <strong>{state.status === 'success' ? 'Congradulations' : 'Oops'} </strong> {state.message}
                </div>
            );
        }
    }
        ...
        return (
            <div>
                 ...
                 <form className="form-signin" onSubmit={handleSubmit(this.handleFormSubmit.bind(this))}>
                    ...
                    <Field name="newPasswordConfirm" component={this.renderField} type="password" label="Confirm New Password" />
