...
class Comments extends Component {
    ...
    renderComment(comment) {
        return (
            <div key={comment._id}>
                <div className="text-justify" dangerouslySetInnerHTML={{_html: comment.content }} />
            <div>
            <span className="span-with-margin f6 text-grey">{comment.authorName}</span>
            <span className="span-with-margin f6 text-grey">•</span>
            <span className="span-with-margin f6 text-grey">{newDate(comment.time).toLocaleString()}</span>
        </div>
        <hr />
        </div>
        );
